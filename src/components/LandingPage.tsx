...
const LandingPage = () => {
  ...
  const userRoles = [
    {
      icon: <UserCog className="h-12 w-12 text-bjj-gold" />,
      title: "Super Admin",
    ...
    {
      icon: <Users className="h-12 w-12 text-bjj-gold" />,
      title: "Coach",
    ...
    {
      icon: <GraduationCap className="h-12 w-12 text-bjj-gold" />,
      title: "Student",
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-bjj-navy via-bjj-navy-light to-bjj-navy">
      {/* Header */}
      <header className="relative z-10 px-6 py-8">
        <div className="container mx-auto flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className="h-12 w-12 bg-bjj-gold rounded-lg flex items-center justify-center">
              <Shield className="h-8 w-8 text-bjj-navy font-bold" />
            </div>
            <div>
              <h1 className="text-2xl font-bold text-white">ADJJA</h1>
              <p className="text-bjj-gold-light text-sm">Academy Management System</p>
            </div>
          </div>
          <Button variant="outline" className="bg-transparent border-bjj-gold text-bjj-gold hover:bg-bjj-gold hover:text-bjj-navy">
            Contact Support
          </Button>
        </div>
      </header>
      ...
      <section className="px-6 py-16 text-center animate-fade-in">
        <div className="container mx-auto max-w-4xl">
          <Badge className="mb-6 bg-bjj-gold/20 text-bjj-gold border-bjj-gold/30">
            Brazilian Jiu-Jitsu Academy Management
          </Badge>
          <h2 className="text-5xl md:text-6xl font-bold text-white mb-6 leading-tight">
            Manage Your BJJ Academy with
            <span className="text-bjj-gold block">Excellence</span>
          </h2>
          ...
          <div className="grid md:grid-cols-3 gap-6 mb-16">
            {userRoles.map((role, index) => (
              <Card key={role.title} className="bg-white/10 border-bjj-gold/20 hover:bg-white/15 transition-all duration-300 hover:scale-105 cursor-pointer">
                <CardHeader className="text-center pb-4">
                  <div className="mx-auto mb-4 p-3 bg-bjj-gold/20 rounded-full w-fit">
                    {role.icon}
                  </div>
                  <div className="flex items-center justify-center gap-2 mb-2">
                    <CardTitle className="text-white text-xl">{role.title}</CardTitle>
                    <Badge variant="secondary" className="bg-bjj-gold text-bjj-navy text-xs">
                      {role.badge}
                    </Badge>
                  </div>
                  <CardDescription className="text-bjj-gold-light">
                    {role.description}
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <ul className="space-y-2">
                    {role.features.map((feature, idx) => (
                      <li key={idx} className="text-gray-300 text-sm flex items-center">
                        <div className="h-1.5 w-1.5 bg-bjj-gold rounded-full mr-2" />
                        {feature}
                      </li>
                    ))}
                  </ul>
                  <Button className="w-full mt-6 bg-bjj-gold hover:bg-bjj-gold-dark text-bjj-navy font-semibold">
                    Login as {role.title}
                  </Button>
                </CardContent>
              </Card>
        ...
        <div className="container mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {features.map((feature, index) => (
              <Card key={feature.title} className="bg-white/10 border-bjj-gold/20 hover:bg-white/15 transition-all duration-300">
                <CardHeader>
                  <div className="mb-4">
                    {feature.icon}
                  </div>
        ...
        <div className="container mx-auto">
          <div className="flex items-center justify-center space-x-3 mb-6">
            <div className="h-8 w-8 bg-bjj-gold rounded flex items-center justify-center">
              <Shield className="h-5 w-5 text-bjj-navy" />
            </div>
            <span className="text-white font-semibold">ADJJA Academy Management</span>
          </div>
